function y = exhaustive_match_recognizing()
  END = 18;
  file_name = [ '../msop_recognizing/1.JPG' ];
  ...
  for nowImage = 1 : END
    file_name = fopen([ '../msop_recognizing/' int2str( nowImage ) '.txt' ], 'r');
    prvData = fscanf( file_name, '%f' );
    prvData = reshape( prvData, 66, [] );
    fclose( file_name );
    for nxtImage = nowImage + 1 : END
      file_name = fopen([ '../msop_recognizing/' int2str( nxtImage ) '.txt' ], 'r');
      ...
      pairFile = fopen( ['../exhaustive_recognizing/' int2str( nowImage ) '_' ...
        int2str( nxtImage ) '.txt'], 'w' );
      num = 0;
      prvPair = 0;
      nowPair = 0;
      for p = 1 : prvSz
